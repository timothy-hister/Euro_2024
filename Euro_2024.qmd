...
```{r games}
games_round_1 = readxl::read_xlsx("inputs/scores/round_1_scores.xlsx", range = "A8:G43", col_names = F) %>%
...
if (params$import_round_1) {
  round_1_preds = map(fs::dir_info("inputs/Round_1")$path, function(wb) {
  ...
  saveRDS(round_1_preds, "results/round_1_preds.Rds")
} else round_1_preds = readRDS("results/round_1_preds.Rds")

if (params$import_round_2) {
  round_2_preds = map(fs::dir_info("inputs/Round_1")$path, function(wb) {
...
```{r scores}
round_1_scores = readxl::read_xlsx("inputs/scores/round_1_scores.xlsx", range = "E8:F43", col_names = F)
  ...
  reactable(
    ...
    details = function(index) {
      ...
      if (last_game > 1) div(
        ...
        div(plotTag(tbl2, alt='hello', width = 500), style = "margin: 30px 45px;")
      ) else div(
        h4(standings_tbl$name[index] %,% "'s games", style = "margin: 10px;"),
        div(tbl1, style = list(margin = "30px 45px"))
      )
...
games %>%
  ...
  group_by(player_id) %>%
  inner_join(players) %>%
  select(round, game_id, date, location, team_1, team_2, name, pred) %>%
  arrange(round, game_id, pred) %>%
  group_by(round, game_id, date, location, team_1, team_2, pred) %>%
  summarise(
  ...
  mutate(pred_blurb = n_players %,,% "people picked" %,,% pred %,% ":" %,,% players %,% ".") %>%
